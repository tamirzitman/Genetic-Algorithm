...
def get_hybrid_mask_list(jobs_amount=constants["JOBS_AMOUNT"]):
    mask_list = []
...
def get_vector_array_by_id(vector_id, file_name=constants["VECTORS_INITIAL_DATA"]):
...
def get_randomize_jobs(jobs_amount=constants["JOBS_AMOUNT"]):
    jobs_list = list(range(1, jobs_amount + 1))
    random.shuffle(jobs_list)
    return jobs_list

def export_list(data_list,file_name = constants["VECTORS_INITIAL_DATA"],clean_csv=False):
        operation = 'a' # append
...
def get_job_process_time(job_id):
    with open(constants["JOBS_DATA"], 'r') as file:
...
def read_csv_to_array(file_path=constants["VECTORS_INITIAL_DATA"]):
...
def get_new_generation(cum_distributed_array,as_is_ratio = constants["AS_IS_RATIO"] ,hybrid_ratio = constants["HYBRID_RATIO"],mutation_rate = constants["MUTATION_RATE"]):
    new_gen_array = []
